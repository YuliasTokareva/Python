...
df['Количество'] = df['Количество'].replace(0, np.nan)
df['Средняя цена'] = df['Продажи'] / df['Количество']
df['Прибыль'] = df['Продажи'] - df['Себестоимость']

with PdfPages('полный_отчет_продаж.pdf') as pdf:
    ...
    all_months = sorted(df['Дата'].dt.to_period('M').drop_duplicates())
    ...
    plt.figure(figsize=(10, 6))
    growth.plot(kind='barh', color='purple')
    plt.title('ТОП-10 товаров по росту продаж')
    plt.xlabel('Прирост выручки')
    pdf.savefig(bbox_inches='tight')
    plt.close()

    avg_sales = df.groupby('точка')['Продажи'].sum().mean()
    fig, ax = plt.subplots(figsize=(8, 4))
    ax.text(0.5, 0.5, f"Средние продажи на точку:\n{avg_sales:,.0f} руб.",
...
print("\nТОП-10 товаров по прогнозу на следующий месяц:")
forecast_list = []
for product in df['товар'].unique():
    prod_data = df[df['товар'] == product].groupby('Дата')['Продажи'].sum().sort_index()
    if len(prod_data) < 2:
        continue
    ...
    forecast_list.append((product, pred))

forecast_df = pd.DataFrame(forecast_list, columns=['товар', 'прогноз'])
top_forecast = forecast_df.nlargest(10, 'прогноз')
print(top_forecast.to_string(index=False))

print("Анализ завершён!Все графики сохранены в 'полный_отчет_продаж.pdf'")